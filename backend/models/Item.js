...
itemSchema.pre('save', function(next) {
  ...
  else if (qty > 0 && qty <= threshold) {  // ✅ Added qty > 0 check
...
itemSchema.post('save', async function(doc) {
  ...
  try {
    ...
    if (doc.stockStatus === 'available' && doc._previousStockStatus !== 'available') {
    ...
    if (!shouldCreateAlert) {
      console.log(`ℹ️  Stock status for ${doc.name} is ${doc.stockStatus} - no alert needed`);
      return;
    }
    
    console.log(`📧 Checking alert for ${doc.name} - Status: ${doc.stockStatus}`);
    ...
    if (existingAlert) {
      console.log(`ℹ️  Unresolved alert already exists for ${doc.name} - Skipping`);
      return;
    }
    ...
    const newAlert = await Alert.create({
      ...
      item: doc._id,
      message: doc.stockStatus === 'out_of_stock' 
...
itemSchema.methods.addRestock = async function(quantity, costPrice, userId, details = {}) {
